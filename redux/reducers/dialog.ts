...
export const dialogReducer = (state = initialState, action: AnyAction): DialogState => {
    switch (action.type) {
        case HYDRATE:
        ...
        case DialogActionTypes.REQUEST_DELETE_DIALOG_SUCCESS: {
            const newItems = state.items.filter(dialogObj => dialogObj.id !== action.payload)
        ...
        //         ...state,
        //         items: state.items.map(dialogObj => {
        //             const userIndex = dialogObj.users.findIndex(user => user.id === action.payload.userId)
