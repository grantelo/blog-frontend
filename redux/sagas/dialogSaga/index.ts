...
import {
  DialogActionTypes,
  RequestCreateDialogAction,
  RequestDeleteDialogAction,
  SetCurrentDialogAction,
} from "../../types/dialog";
import {
  requestCreateDialogError,
  requestCreateDialogSuccess,
  requestDeleteDialogError,
  requestDeleteDialogSuccess,
  requestDialogsError,
...
export function* requestCreateDialog({ payload }: RequestCreateDialogAction) {
  try {
    ...
    const response: AxiosResponse<IDialog> = yield call(
      Api().dialog.create,
      payload
    );
...
export function* setCurrent({ payload }: SetCurrentDialogAction) {
  const socket: Socket = yield select<(state: RootState) => Socket>(
